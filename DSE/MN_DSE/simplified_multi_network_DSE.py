...
class multi_network_DSE:
                    ...
                    for workload in sp_space[sp_id]:
                        ...
                        for layer, items in fitness.items():
                            if comm_tag == True:
                                ...
                                commPerf = 0
                            sp_workload_fitness[sp_id][workload]['L'] += items['L'] + commPerf
    ...
    def record_sample(self, workload_code_list, sp_Nchip_list_list):
        line_workload = ''
        for workload_code in workload_code_list:
            line_workload += '-\n'
            for workload, [tp_id, sp_id] in workload_code.items():
                line_workload += "  {} :\n".format(workload)
                line_workload += "    - {}\n".format(tp_id)
                line_workload += "    - {}\n".format(sp_id)
        file_workload = "./result/workload_record.txt"
        f = open(file_workload, 'w')
        print(line_workload, file=f)
...
if __name__ == '__main__':
    ...
    mesh_f, cmesh_f, ours_f, mesh_latency_tp_sp, cmesh_latency_tp_sp, ours_latency_tp_sp = MNN_Engine.evoluation_temporal_spatial()

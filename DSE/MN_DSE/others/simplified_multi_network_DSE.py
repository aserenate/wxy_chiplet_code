...
def get_workload_partition(architecture, nn_list, ratio, objective):
    ...
    for nn_name in nn_list:
        workload_dict[nn_name] = getWorkloadPartition(nn_name, 'param_mux_compute', ratio)
...
class multi_network_DSE:
    def __init__(self, architecture, chiplet_num, mem_num, nn_list, Optimization_Objective, tp_TH, sp_TH, topology='ours', partition_ratio=0.5, BW_tag = 1, debug_tag = 0, layout_mapping_method = 'balance'):
        ...
        self.workload_schedule_TH = 10000    # max workload schedule method num
    ...
    # chiplet_partition_dict : 
        ...
        ##   max_sp_num:[[],[],...] }
    # -------------------------------------------------
    def getChipletPartionDict(self, max_sp_num, min=0):
        par_per_sp_TH = 100
        par_num = 0
        unchange_TH = 50
        unchange_times = 0
        block_size = 1
        total_block_num = math.ceil(self.chiplet_num / block_size)
        
        chiplet_partition_dict = {}
        for i in range(min, max_sp_num):
            sp_num = i + 1
            par_num = 0
            unchange_times = 0
        ...
        for sp_num, partitions in chiplet_partition_dict.items():
            for i in range(len(partitions)):
                ...
                for j in range(sp_num):
                    chiplet_partition_dict[sp_num][i][j] *= block_size
                    ...
                    for workload in sp_space[sp_id]:
                        ...
                        for layer, items in fitness.items():
                            if comm_tag == True:
                                ...
                                # commPerf_mesh = comm_performance(ifmapD, weightD, ofmapD, parallel, 'mesh', self.route_table, self.linkBW, self.links)
                                commPerf_ours = comm_performance(ifmapD, weightD, ofmapD, parallel, 'ours', None, None, None)
                            else:
                                commPerf_ours = 0
                            sp_workload_fitness[sp_id][workload]['L'] += items['L'] + commPerf_ours
    ...
    def getWorkloadScheduleCodeList_random(self, tp_ratio, sp_ratio, TH):
        ...
        nn_sp_partitions = get_total_group(app_num, sp_TH)
    ...
    def evoluation_temporal_spatial(self):
        ...
        mesh_f, ours_f = self.finalFitness()
        return mesh_f, ours_f
    ...
    def finalFitness(self):
        ...
        return mesh_fitness, ours_fitness
...
if __name__ == '__main__':
    ...
    mesh_f, ours_f = MNN_Engine.evoluation_temporal_spatial()
